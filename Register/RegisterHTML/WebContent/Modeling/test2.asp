...
next
	...
	if SensorID ="s" then 
		...
		if outputCount="0s" then
			...
			if fs.FileExists(filez) then
				...
				call Generation_File("content.xml","/WebContent/Modeling/SimpleRegister",content)
			end if
	...
	call Generation_File(SensorID_Name&".xml","/WebContent/Modeling/SimpleRegister",content) '在xml目录下生成编码为utf-8的content.xml文
