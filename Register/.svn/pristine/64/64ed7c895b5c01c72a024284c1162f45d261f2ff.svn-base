...
<%
	...
	top = top&"                      <swe:value>http://202.121.197.111:84/SHUSVI_SOS/sos</swe:value>"&vbcrlf
	top = top&"                    </swe:Text>"&vbcrlf
	top = top&"                  </swe:field>"&vbcrlf
...
'	response.Write("pass1:"&pass)
	...
	filez=all_tree2&"\"&files
		...
		Response.Write "<script>window.location.reload();alert('"""&SensorID_Name&"""已经存在！请修改ID或对原xml文档进行修改！');history.back();</script>"		
